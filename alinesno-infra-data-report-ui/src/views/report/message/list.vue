 ...
 * @date 2022-05-09 09:33:44
  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="MqMessageList" @selection-change="handleSelectionChange" :default-sort = "{prop: 'fileIndex', order: 'ascending '}" @sort-change="changeTableSort">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="业务模型" align="left" width="130" prop="modelNameLabel" show-overflow-tooltip :sortable="'custom'"  />
      <el-table-column label="消息主题" align="left" width="130" prop="kafkaTopiceLabel" show-overflow-tooltip :sortable="'custom'"  />
      <el-table-column label="上报文件" align="left" prop="fileNameLabel" show-overflow-tooltip :sortable="'custom'"  />
      <el-table-column label="目标库" align="left" prop="messageTypeLabel" show-overflow-tooltip :sortable="'custom'"  />
      <el-table-column label="序号" align="left" prop="fileIndex" show-overflow-tooltip :sortable="'custom'"  width="100" />
<!--      <el-table-column label="消息内容" align="left" prop="data" :sortable="'custom'"  >-->
...
<!--        </template>-->
<!--      </el-table-column>-->
      <el-table-column label="上报时间" align="center" prop="addTime" width="170" :sortable="'custom'"  >
        <template #default="scope">
          <span>{{ parseDatetime(scope.row.addTime) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="上报状态" align="left" prop="reportStatusLabel" show-overflow-tooltip :sortable="'custom'"  width="98" />
      <el-table-column label="已发总线" align="left" prop="ifSendBusLabel" show-overflow-tooltip :sortable="'custom'"  width="98" />
      <el-table-column label="操作" width="260" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          >删除</el-button>
          <el-button
            v-show="scope.row.ifSendBus==0"
            size="mini"
            type="text"
            icon="el-icon-right"
            @click="handleSendOne(scope.row)"
          >单条发送</el-button>
          <el-button
            v-show="scope.row.ifSendBus==0"
            size="mini"
...
import {
  ...
  updateMqMessage,
  exportMqMessage,messageSendToBus } from "@/api/report/Message";
...
const { pickerOptions, queryParams, queryParamsConfig, form, rules, jsonData, Params, ParamsConfig  } = toRefs(data);
...
function  handleSendOne(row){
  messageSendToBus(row.id, row.fileId, 1).then(res =>{
...
function  handleSendFile(row){
  messageSendToBus(row.id, row.fileId, 0).then(res =>{
    proxy.$modal.msgSuccess(res)
    getList()
  });
}
...
function  changeTableSort(column) {
  ...
  MqMessageList.value = MqMessageList.sort(
