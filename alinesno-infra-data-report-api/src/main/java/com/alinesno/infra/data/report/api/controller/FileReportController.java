...
/**
 * 处理与FileReportEntity相关的请求的Controller。
 * 继承自BaseController类并实现IFileReportService接口。
 *
 * @author LuoXiaoDong
 * @version 1.0.0
 */
...
@RequestMapping("/api/infra/data/report/file_report")
public class FileReportController extends BaseController<FileReportEntity, IFileReportService> {

    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(FileReportController.class);

    @Autowired
    private IFileReportService service;

    /**
     * 获取FileReportEntity的DataTables数据。
     *
